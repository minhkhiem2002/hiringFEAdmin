...
import { getBanRequest } from "../../../redux/actions/Admin/banActions";
import DeleteModal from './ModalBan/DeleteModal'
import AddModal from "./ModalBan/AddModal";
...
function BanManage() {
  ...
  const { data, addSuccess, editSuccess, deleteSuccess, isLoading } =
    useSelector((state) => state.getAllBan);

    console.log(data)
  useEffect(() => {
    dispatch(getBanRequest());
    setInfoDetail(null);
  ...
  useEffect(() => {
    if (data) {
      setDataTableStaff(data.banLists);
    }
  }, [data]);
